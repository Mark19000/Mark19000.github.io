...
export const projectContent: ProjectContent[] = [
  ...
  },
  {
    ...
    stack: 'ARM64 · Linux 5.15 · Clang · Device Tree · AVB',
    visual: 'linux',
    image: '/projects/linux-tab.png',
    repository: 'https://github.com/Mark19000/sm-x510-linux-bringup',
    hardware: ['Samsung SM-X510', 'Exynos S5E8835', 'ARM64', 'U12 / EZE4'],
    software: ['Linux kernel 5.15', 'Clang 21', 'Device Tree', 'Initramfs', 'AVB', 'Lima'],
    es: {
      name: 'Linux bring-up · Galaxy Tab S9 FE', status: 'Fase offline validada · Hardware NO-GO', label: 'Linux platform engineering',
      intro: 'Un entorno reproducible para investigar el arranque de Linux en una Samsung Galaxy Tab S9 FE con SoC Exynos S5E8835, desde las fuentes del kernel hasta el primer experimento físico observable.',
      body: 'El proyecto reconstruye y audita la cadena de arranque sin tocar todavía la tablet: compila el kernel Samsung U11, genera initramfs mínimos, compara Device Trees y módulos con el firmware U12/EZE4 y bloquea cualquier escritura física mientras no exista una ruta de observación y recuperación suficientemente segura.',
      challenge: 'Trabajar con fuentes U11 frente a hardware U12/EZE4, una cadena AVB específica de Samsung y canales de depuración todavía no demostrados, evitando confundir “compila” con “es compatible” o “arranca”.',
      built: 'Un pipeline ARM64 offline que produce builds reproducibles del kernel con Clang 21, aplica parches auditables, construye DTB/DTBO e initramfs y verifica sus resultados mediante pruebas host, comparación semántica y manifiestos de hashes.',
      why: 'Quería convertir un port de Linux difícil de observar en un experimento de ingeniería repetible: cada hipótesis, diferencia binaria y condición de seguridad debe poder verificarse antes de modificar el dispositivo.',
      learned: 'En el bring-up de hardware cerrado, la reproducibilidad y una buena estrategia de observación importan tanto como el kernel. Saber cuándo mantener un NO-GO también forma parte del trabajo técnico.',
    },
    en: {
      name: 'Linux bring-up · Galaxy Tab S9 FE', status: 'Offline phase validated · Hardware NO-GO', label: 'Linux platform engineering',
      intro: 'A reproducible environment for investigating Linux boot on a Samsung Galaxy Tab S9 FE with an Exynos S5E8835 SoC, from kernel sources to the first observable hardware experiment.',
      body: 'The project reconstructs and audits the boot chain without touching the tablet yet: it builds Samsung’s U11 kernel, produces minimal initramfs images, compares Device Trees and modules against U12/EZE4 firmware, and blocks physical writes until observation and recovery paths are sufficiently safe.',
      challenge: 'Working with U11 sources against U12/EZE4 hardware, Samsung’s AVB chain and unproven debugging channels—without confusing “builds successfully” with “compatible” or “boots”.',
      built: 'An offline ARM64 pipeline producing reproducible Clang 21 kernel builds, auditable patches, DTB/DTBO and initramfs artifacts, verified through host tests, semantic comparisons and hash manifests.',
      why: 'I wanted to turn a difficult-to-observe Linux port into a repeatable engineering experiment: every hypothesis, binary difference and safety condition should be verifiable before modifying the device.',
      learned: 'For closed-hardware bring-up, reproducibility and a sound observation strategy matter as much as the kernel itself. Knowing when to preserve a NO-GO is also part of the engineering.',
    },
  },
  {
